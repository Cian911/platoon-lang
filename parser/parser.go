...
// We use iota to give the following consts
// incrementing numbers as values.
// This will help us to satisfy operator order of precendence
const (
	_ int = iota
	LOWEST
	EQUALS      // ==
...
type Parser struct {
	...
	// Check if the appropriate map (infix or prefix)
	// has the parsing function for the current token.
...
func New(l *lexer.Lexer) *Parser {
	...
	// Read nextToken twice, so curToken and nextToken are set
	p.nextToken()
	p.nextToken()
...
func (p *Parser) noPrefixParseFnError(t token.TokenType) {
	msg := fmt.Sprintf("no prefix parse func found for %s", t)
	p.errors = append(p.errors, msg)
}
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	value, err := strconv.ParseInt(ilt.Token.Literal, 0, 64)
	if err != nil {
